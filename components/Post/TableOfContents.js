...
export default function TableOfContents ({ blockMap, frontMatter, pageTitle }) {
  ...
  return (
    <div
      className='hidden xl:block xl:fixed ml-4 text-sm text-neutral-500 dark:text-neutral-400 whitespace'
    >
      {pageTitle && (
        <Link
          passHref
          ...
          scroll={false}
          className='block -ml-6 mb-2 p-2 hover:bg-neutral-200 hover:dark:bg-neutral-700 rounded-lg'
        >
      ...
      {nodes.map(node => (
        <div key={node.id} className='px-2 hover:bg-neutral-200 hover:dark:bg-neutral-700 rounded-lg'>
          <a
